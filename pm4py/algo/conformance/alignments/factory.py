...
def apply(obj, petri_net, initial_marking, final_marking, find_all_opt_alignments=False, parameters=None,
          version=VERSION_STATE_EQUATION_A_STAR):
    if isinstance(obj, pm4py.objects.log.log.Trace):
        return apply_trace(obj, petri_net, initial_marking, final_marking, find_all_opt_alignments, parameters, version)
    else:
        return apply_log(log_converter.apply(obj, parameters, log_converter.TO_TRACE_LOG), petri_net, initial_marking,
                         final_marking, find_all_opt_alignments, parameters, version)
