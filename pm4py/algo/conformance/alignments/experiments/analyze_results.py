...
def start_bpi_2019():
    ...
    algo_result_keys_to_description_string = {
        'a_star_from_scratch_without_heuristic': 'AS',
        'a_star_from_scratch_with_heuristic': 'AS-H',
        'incremental_a_star_without_heuristic': 'IAS',
        'incremental_a_star_with_heuristic': 'IAS-H',
        'online_conformance': 'OCC',
        'online_conformance_window_1': 'OCC-1',
        'online_conformance_window_2': 'OOC-2',
        'online_conformance_window_5': 'OCC-5',
    }
    ...
    algo_result_keys_to_description_string = {
        ...
        'online_conformance': 'OCC',
        'online_conformance_window_1': 'OCC-1',
        'online_conformance_window_2': 'OCC-2',
        'online_conformance_window_5': 'OCC-5',
        'online_conformance_window_10': 'OCC-10',
    }
    ...
    path_to_files = os.path.join("C:\\", "Users", "Daniel", "Desktop", "master_thesis", "experiments", "results",
                                 "conformance_checking_challeng_all_traces")
    ...
    for res_file in result_files:
        ...
        #
        #         key = 'incremental_a_star_with_heuristic'
        #         number_recalculations = []
        #         total_number = []
        #         for trace in results:
        #             recalculations = 1
        #             total = 0
        #             for intermediate in trace[key]['intermediate_results']:
        #                 recalculations = recalculations + intermediate['number_solved_lps'] - intermediate['queued_states']
        #                 total = total + intermediate['number_solved_lps']
        #             number_recalculations.append(recalculations)
        #             total_number.append(total)
        #
        #         number_recalculations = sum(number_recalculations)
        #         total_number = sum(total_number)
        #         print(total_number/len(results))
        #         print(number_recalculations/len(results))
        # return
        # just to check if costs are the same #########################################################################
        # for trace in results:
        #     cost = trace['incremental_a_star_with_heuristic']['cost']
        #     for key in algo_result_keys:
        #         if trace[key]['cost'] == cost:
        #             pass
        #         else:
        #             print('\nfound cost difference')
        #             print(trace['trace_index'])
        #             print(key)
        #             print(round(cost / 1000))
        #             print(round(trace[key]['cost'] / 1000))
        # return
...
def plot_time_bar_chart(algo_result_keys, results, path_to_files, res_file, description_algos):
    ...
    # plot_time_per_algorithm(tuple(a_star_computation_time_without_heuristic),
    #                         tuple(computation_time_heuristic),
    #                         tuple(description_algos_subset), number_solved_lps, path_to_store=filename, svg=plot_svg)
...
def bar_plot_miscellaneous(algo_result_keys, results, path_to_files, res_file, description_algos, attribute):
    ...
    # generate_simple_bar_plot(tuple(res), tuple(description_algos_subset), filename, attribute, svg=plot_svg)
...
def bar_plot_cost(algo_result_keys, results, path_to_files, res_file, description_algos, attribute):
    ...
    # time plot for all algorithm variants
    filename = os.path.join(path_to_files, res_file + '_' + attribute)
    generate_simple_bar_plot(tuple(res), tuple(description_algos), filename, attribute="cost difference (\%)", svg=plot_svg)
...
if __name__ == '__main__':
    # plot_petri_net('C:\\Users\\Daniel\\Desktop\\master_thesis\\experiments\\data\\bpi_challenge_2019\\petri_net_3.pnml')
    start_bpi_2019()
