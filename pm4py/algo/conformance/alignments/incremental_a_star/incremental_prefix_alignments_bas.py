...
def apply(trace, petri_net, initial_marking, final_marking, parameters=None, debug_print=True, window_size=0):
    ...
    for event in trace:
        ...
        res = __calculate_prefix_alignment_for_next_event(petri_net, sync_prod, sync_im, sync_fm, cost_function,
                                                          SKIP, prefix_alignment, trace, activity_key, window_size)
...
def __calculate_prefix_alignment_for_next_event(process_net, sync_net, start_marking, final_marking, cost_function,
                                                skip,
                                                prefix_alignment, trace, activity_key, window_size, debug_print=True):
    start_time = time.time()
    ...
    for t in process_net.transitions:
        if t.label == activity_name:
            ...
            synchronous_move_transition_enabled = None
            synchronous_move_transition = None

            for t_s in sync_net.transitions:
                if t_s.label[0] == t_s.label[1] == activity_name:
                    # there is a corresponding synchronous move transition in the synchronous product net
                    synchronous_move_transition = t_s
                    synchronous_move_transition_enabled = t_s in petri.semantics.enabled_transitions(sync_net,
                                                                                                     start_marking)
                    # since there can be only one synchronous move transition for the given activity we can break here
                    break

            if synchronous_move_transition_enabled and synchronous_move_transition:
                # ADD SYNCHRONOUS MOVE
                if debug_print:
                    print("ADD SYNCHRONOUS MOVE ")
                new_marking = petri.semantics.execute(synchronous_move_transition, sync_net, start_marking)
                ...
                prefix_alignment = prefix_alignment + [{"marking_before_transition": start_marking,
                ...
                res = __search(sync_net, start_marking, final_marking, cost_function, skip, cost_so_far)
    ...
    for t in sync_net.transitions:
        if is_log_move(t) and t.label[0] == activity_name and petri.semantics.is_enabled(t, sync_net, start_marking):
            new_marking = petri.semantics.execute(t, sync_net, start_marking)
            prefix_alignment = prefix_alignment + [{"marking_before_transition": start_marking,
                                                    "label": t.label,
                                                    "name": t.name,
                                                    "cost_so_far": cost_function(t) + cost_so_far,
                                                    "marking_after_transition": new_marking}]
            return {'alignment': prefix_alignment,
                    'cost': cost_function(t) + cost_so_far,
                    'visited_states': 0,
                    'queued_states': 0, 'traversed_arcs': 0,
                    'total_computation_time': time.time() - start_time,
                    'heuristic_computation_time': 0}
        else:
            # this code should be never reached if everything is okay
